...
if [[ -d "backend" ]]; then
  try_candidates "backend exists but does not expose a supported Maven or npm lint command." \
    "run_maven_goal backend validate" "run_npm_script backend lint"
  ran_any=true
fi

if [[ -d "frontend" ]]; then
  try_candidates "frontend exists but does not expose a supported npm or Maven lint command." \
    "run_npm_script frontend lint" "run_maven_goal frontend validate"
  ran_any=true
fi
