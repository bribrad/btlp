...
if [[ -d "backend" ]]; then
  try_candidates "backend exists but does not expose a supported Maven or npm test command." \
    "run_maven_goal backend test" "run_npm_script backend test"
  ran_any=true
fi

if [[ -d "frontend" ]]; then
  try_candidates "frontend exists but does not expose a supported npm or Maven test command." \
    "run_npm_script frontend test" "run_maven_goal frontend test"
  ran_any=true
fi
